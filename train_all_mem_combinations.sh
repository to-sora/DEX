...
# Create directories
mkdir -p logs
mkdir -p .checkpoints_mem

# Function to check if a combination has been completed
is_completed() {
    local task=$1
    local agent=$2
    if [ -f ".checkpoints_mem/${task}_${agent}.done" ]; then
        return 0 # True
    else
        return 1 # False
    fi
...
mark_completed() {
    local task=$1
    local agent=$2
    touch ".checkpoints_mem/${task}_${agent}.done"
}
...
completed_count=$(ls .checkpoints_mem/*.done 2>/dev/null | wc -l)
